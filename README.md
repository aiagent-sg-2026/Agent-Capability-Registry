...
Continuous Factory V1 maintains an append-only opportunity queue, performs one bounded discovery refill when exhausted, and materializes only reviewed data-only markdown/JSON packages under state. No production deployment is claimed. Curated seeds are reconciled append-only into existing state on every cycle, so new product capabilities reach long-running factories without rewriting prior queue history. Generation-format and review failures receive up to three bounded attempts before the opportunity is consumed, preventing a single model-format failure from permanently dropping work while remaining fail-closed. Future candidates also pass a low-cost quality gate: a compact 0.7–2.2 KB README with six required engineering sections and exactly 3 concise meaningful eval cases. This raises depth without loading large repositories or increasing context size. The first high-value expansion adds ERP read-first investigation, PostgreSQL report-query review, API contract review, webhook delivery debugging, and ERP data-lineage tracing.
