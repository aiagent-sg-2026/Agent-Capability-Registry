...
import {createFactoryCatalog} from '@acr/registry-api';
...
try{
  if(cmd==='search'){
    ...
    if(!cmd||!p)throw Error('usage: cap validate|verify|inspect|install <packagePath> [--target dir] | cap search <query> [flags] | cap info <name> [flags]');
